...
module.exports.handler = function(argv) {
  var fixedEmulatorSerial = argv.serial

  // As argv.serial for android emulator looks:
  //    <hostname>-<port>
  //   We need to fix serial to be understandable for adb. So <hostname> it will be changed to "emulator"
  if (fixedEmulatorSerial.indexOf('-') !== -1) {
    fixedEmulatorSerial = 'emulator-' + fixedEmulatorSerial.split('-').pop(-1)
  }

  return require('../../units/device')({
    serial: argv.serial
  , fixed_serial: fixedEmulatorSerial
