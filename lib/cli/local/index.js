...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('auth-token', {
      describe: 'A token for the REST API.'
      , type: 'string'
      , default: 'kute kittykat'
    })
    .option('auth-type', {
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--adb-keys-dir-host', argv.adbKeysDirHost
        , '--rest-url', util.format('http://%s:%d/', argv.publicIp, argv.port)
      ].concat(JSON.parse(argv.storageOptions)))
      // TODO not the nicest solution to propagate save-dir to testexecutor
