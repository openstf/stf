...
module.exports.handler = function(argv) {
  ...
  , fork: function(device, ports, whiteList) {
      ...
      // In package.json, yargs' option 'duplicate-arguments-array' is set to FALSE,
      // so the form of arguments '-x 1 -x 2' would not be converted an array, which should be changed to
      // to the form of '-x 1 2 xxx'
      if (whiteList.length > 0) {
        args = args.concat('--cleanup-white-list)
                   .concat(whiteList)        
      }
