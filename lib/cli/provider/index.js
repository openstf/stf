...
module.exports.handler = function(argv) {
  ...
  , fork: function(device, ports) {
      ...
      /* For android emulators, which serial number looks:
        emulator-<port>
        we can observe, that if two emulators on different providers works on on the same port, the first serial number
        form provider A it will be overwritten by second emulator on provider B.
        To avoid of this situation, the emulator(key android name) it will be replaced by provider hostname
      */
      var os = require('os')
      var serial = device.id
      if (serial.indexOf('-') !== -1) {
        serial = os.hostname() + '-' + serial.split('-').pop(-1)
      }
