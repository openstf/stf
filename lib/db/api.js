  ...
  return db.run(r.table('devices').get(serial).update(data))
    .then(function(stats) {
      if (stats.skipped) {
        ...
        data.controlGroups = ['default'] //add addition fields in devices DB
...
dbapi.setDeviceInWipeOut = function(serial) {
  return db.run(r.table('devices').get(serial).update({
    wipeout: true
    , presenceChangedAt: r.now()
  }))
...
dbapi.setDeviceDoneWipeOut = function(serial) {
  return db.run(r.table('devices').get(serial).update({
    wipeout: false
    , presenceChangedAt: r.now()
  }))
...
dbapi.addGroup = function (serial, group) {
  db.run(r.table('devices').get(serial)).then(function (device) {
    if ('controlGroups' in device){
      var oldGroups = device['controlGroups']
    }else{
      var oldGroups = []
    }
    if (oldGroups.indexOf(group) < 0){
      oldGroups.push(group)
      db.run(r.table('devices').get(serial).update({
        controlGroups: oldGroups
      }))
    }
  })
}

dbapi.removeGroup = function (serial, group) {
  db.run(r.table('devices').get(serial)).then(function (device) {
    var oldGroups = device['controlGroups']
    var index = oldGroups.indexOf(group);
...
dbapi.addGroupUser = function (email, group) {
  db.run(r.table('users').get(email)).then(function (user) {
    if ('userGroups' in user){
      var oldGroups = user['userGroups']
    }else{
      var oldGroups = []
    }
    if (oldGroups.indexOf(group) < 0){
      oldGroups.push(group)
      db.run(r.table('users').get(email).update({
        userGroups: oldGroups
      }))
    }
  })
}

dbapi.removeGroupUser = function (email, group) {
  db.run(r.table('users').get(email)).then(function (user) {
    var oldGroups = user['userGroups']
    var index = oldGroups.indexOf(group);
    oldGroups.splice(index, 1);
...
dbapi.loadGroups = function () {
  return db.run(r.table('adminGroups'))
}

dbapi.saveGroup = function (name) {
  return db.run(r.table('adminGroups').insert({
    name: name
  }))
}

dbapi.deleteGroup = function (name) {
  db.run(r.table('adminGroups').get(name).delete())
}
...
dbapi.uploadAvatar = function (email, file) {
  db.run(r.table('users').get(email).update({
    file: file
  }))
}
