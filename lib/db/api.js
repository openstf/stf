...
dbapi.loadSchedule = function(serial) {
  return db.run(r.table('schedules').filter(r.row('serial').eq(serial)));
}

dbapi.loadScheduledDevices = function(from, to) {
  return db.run(r.table('schedules').filter(
    r.row('start').lt(to).and(
      r.row('start').gt(from).or(r.row('start').eq(to)))
  ))
}
