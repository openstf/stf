...
dbapi.markStartOfDeviceUsage = function(leaseId, ownerEmail, deviceMetadata, startsFrom, endsAt) {
  return db.run(r.table('users').get(ownerEmail))
    .then(function(user) {
...
dbapi.markEndOfDeviceUsage = function(leaseId, endsAt) {
  return db.run(r.table('deviceUsageStats')
    .filter({leaseId: leaseId})
    .orderBy(r.desc('startsFrom'))
    .limit(1).update({
      endsAt: endsAt
    }))
}
