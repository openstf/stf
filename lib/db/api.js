...
dbapi.setDeviceEmulatorName = function(serial, emulator_name) {
    if (emulator_name.length > 0 ) {
          log.info('Set "' + emulator_name + '" name to device with "' + serial + '" serial')
      return db.run(r.table('devices').get(serial).update({
...
dbapi.saveDeviceIdentity = function(serial, identity) {
  ...
  if (typeof(identity.emulatorName) !== 'undefined'){
    if (identity.emulatorName.length > 0) {
      ...
      data['emulatorArgs'] = '-wipe-data -no-boot-anim -no-window'
    }
  }
...
dbapi.getProviderInfo = function(provider_name) {
    return db.run(r.table('provider').get(provider_name))
}

dbapi.saveProviderInfo = function(provider_name , channel)
{
  var data = {
    name: provider_name
  , channel: channel
  }
  return db.run(r.table('provider').get(provider_name).update(data))
    .then(function(stats) {
      if (stats.skipped) {
...
dbapi.updateEmulatorStartArgs = function(serial, args_text) {
  db.run(r.table('devices').get(serial).update({emulatorArgs : args_text}))
}
