...
dbapi.loadNearestReservation = function(serial) {
  return db.run(r.table('schedules').filter({
    serial: serial
  })
 .filter(
   r.row('end').gt(r.now())
 ).orderBy('start').limit(1))
}

dbapi.loadNextReservation = function(id) {
  return db.run(r.table('schedules').get(id))
    .then(function(reserved_info){
      // todo: revise ReQL
    })
}
dbapi.getSchedule = function(id) {
  return db.run(r.table('schedules').get(id))
}
