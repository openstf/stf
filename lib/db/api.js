...
dbapi.watchTable = function(tableName) {
  return db.run(r.table(tableName).changes(), function(err, cursor) {
    if (err) {
      throw err
    }
    return cursor
...
dbapi.loadCurrentSchedule = function(serial) {
  return db.run(
    r.table('schedules').filter(function(schedule) {
      return schedule('serial').eq(serial).and(
        schedule('end').gt(r.now())
          .and(schedule('start').lt(r.now()))
      )
    }))
}
dbapi.loadNextSchedule = function(serial) {
  return db.run(
    r.table('schedules')
      .orderBy({index: r.asc('start')})
      .filter(function(schedule) {
        return schedule('serial').eq(serial).and(
          schedule('start').gt(r.now()))
      })
      .limit(1)
  )
}
