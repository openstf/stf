...
dbapi.setDeviceEmulatorName = function(serial, emulatorName) {
    log.info("Update Emulator Name by: " + emulator_name)
    log.info("Data inside db : " + JSON.stringify(db.run(r.table('devices').get(serial))) )
    return db.run(r.table('devices').get(serial).update({
