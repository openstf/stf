...
// schedule api
dbapi.loadStartSchedules = function(from, to) {
  return db.run(r.table('schedules')
                .between(from, to, {index: 'start'}));
}
dbapi.loadEndSchedules = function(from, to) {
  return db.run(r.table('schedules')
                .between(from, to, {index: 'end'}));
}
dbapi.loadSchedules = function(serial, from, to) {
  return db.run(r.table('schedules').filter(function(schedule){
    return schedule('serial').eq(serial).and(
      schedule('end').gt(from).and(
        schedule('start').lt(to)))
    })
  )
}

dbapi.insertDeviceSchedule = function(email, data) {
  return db.run(r.table('schedules').insert({
    email: email
  , serial: data.serial
  , start: data.start
  , end: data.end
...
dbapi.updateDeviceSchedule = function(email, data) {
  return db.run(r.table('schedules').filter({
    id: data.id
  , serial: data.serial
  , email: email
  }).update({
...
dbapi.deleteDeviceSchedule = function(id, email) {
  return db.run(r.table('schedules').get(id).delete())
}
