...
module.exports = function(conn) {
  ...
  function createIndex(table, index, options) {
    ...
    function waitForIndex(table, index) {
      return Promise.resolve(r.table(table).getAll(true, {
          index: index
        }).run(conn))
        .catch(function() {
          return Promise.resolve(waitForIndex(table, index))
        })
    }
    ...
    return rTable.indexCreate.apply(rTable, args).run(conn)
      ...
      .then(function() {
        ...
        return waitForIndex(table, index)
  ...
  function createTable(table, options) {
    ...
    return r.tableCreate(table, tableOptions).run(conn)
      ...
      .then(function() {
        if (options.indexes) {
          // create simple index first, then complex index
          var simple_indexes = Object.keys(options.indexes).filter(function(index) {
            return !options.indexes[index]
          })
          log.info('Processing simple indexes', simple_indexes)

          return Promise.all(simple_indexes.map(function(index) {
              return createIndex(table, index, options.indexes[index])
            }))
            .then(function() {
              var non_simple_indexes = Object.keys(options.indexes).filter(function(index) {
                return !!options.indexes[index]
              })
              log.info('Processing non-simple indexes', non_simple_indexes)

              return Promise.all(non_simple_indexes.map(function(index) {
                return createIndex(table, index, options.indexes[index])
              }))
            })
        }
