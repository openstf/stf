...
function updateUserPrivilege(req, res) {
  const email = req.swagger.params.email.value
  const privilege = req.swagger.params.privilege.value

  dbapi.updateUserPrivilege(email, privilege, req.user.ip).then(function(stats) {
    if (!stats.replaced) {
      return apiutil.respond(res, 200, 'Unchanged (user privilege)', {user: {}})
    }

    return apiutil.respond(res, 200, 'Updated (user privilege)', {
      user: apiutil.publishUser(stats.change)
    })
  })
  .catch(function(err) {
    apiutil.internalError(res, 'Failed to update user privilege: ', err.stack)
  })
}
