...
module.exports = function(options) {
  ...
  app.get('/app/api/v1/device-schedule/:serial/:from/:to', function(req, res){
    var from = new Date(parseInt(req.params.from, 10))
    var to = new Date(parseInt(req.params.to, 10))
    dbapi.loadSchedules(req.params.serial, from, to)
      .then(function(cursor){
        return Promise.promisify(cursor.toArray, cursor)()
      ...
      .catch(function(err) {
        log.error('Failed to load device schedules serial["%s"] from[%s] to[%s]: ', req.params.serial, req.params.from, req.params.to)
        log.error(err.stack)
        res.json(500, {
          success: false
        })
