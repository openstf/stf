  ...
  .define(function(options, adb, router, push) {
    ...
    router.on(wire.InstallMessage, function(channel, message) {
      ...
      function pushApp() {
        ...
        return adb.push(options.fixed_serial, source, target)
          .timeout(10000)
      ...
      pushApp()
        .then(function(apk) {
          ...
          return promiseutil.periodicNotify(
              adb.installRemote(options.fixed_serial, apk)
                .timeout(60000 * 5)
                .catch(function(err) {
                  switch (err.code) {
                  ...
                  case 'INSTALL_FAILED_VERSION_DOWNGRADE':
                    ...
                    return adb.uninstall(options.fixed_serial, pkg)
                      .timeout(15000)
                      .then(function() {
                        return adb.installRemote(options.fixed_serial, apk)
                          .timeout(60000 * 5)
                      })
                  default:
        ...
        .then(function() {
          if (message.launch) {
            if (manifest.application.launcherActivities.length) {
              ...
              return adb.startActivity(options.fixed_serial, launchActivity)
                .timeout(30000)
            }
          }
        })
    ...
    router.on(wire.UninstallMessage, function(channel, message) {
      ...
      adb.uninstall(options.fixed_serial, message.packageName)
        .then(function() {
          push.send([
            channel
          , reply.okay('success')
