      ...
      bin: new Resource({
        src: pathutil.requiredMatch(abi.all.map(function(supportedAbi) {
          return pathutil.module(util.format(
            'minicap-prebuilt/prebuilt/%s/bin/%s/minicap%s'
          , supportedAbi
          , properties["ro.build.characteristics"].indexOf('emulator') >= 0 ? 'emulator' : ''
          , abi.pie ? '' : '-nopie'
          ))
        }))
      , dest: [
        ...
        src: pathutil.requiredMatch(abi.all.reduce(function(all, supportedAbi) {
          return all.concat([
            pathutil.module(util.format(
              'minicap-prebuilt/prebuilt/%s/lib/android-%s/%s/minicap.so'
            , supportedAbi
            , sdk.release
            , properties["ro.build.characteristics"].indexOf('emulator') >= 0 ? 'emulator' : ''
            ))
          , pathutil.module(util.format(
            ...
            , supportedAbi
            , sdk.previewLevel
            , options.serial.indexOf('emulator') >= 0 ? 'emulator' : ''
            ))
          , pathutil.module(util.format(
              'minicap-prebuilt/prebuilt/%s/lib/android-%s/%s/minicap.so'
            , supportedAbi
