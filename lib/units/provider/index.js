...
module.exports = function(options) {
  ...
  client.trackDevices().then(function(tracker) {
    ...
    dbapi.saveProviderInfo(os.hostname(), solo)
      ...
      function spawn() {
        ...
        return resolver.promise
          ...
          .catch(Promise.CancellationError, function() {
            if (!didExit) {
              log.info('Gracefully killing device worker "%s"', addHostnameToEmulatorSerial(device.id))
    ...
    sub.on('message', wirerouter()
      ...
      .on(wire.AVDRestart, function(channel, message){
            if (message.hostname === os.hostname()){
              log.info('AndroidSDK AVD: Restarting emulator: "' + message.emulator_name + '" at "' + os.hostname() +
              '" machine')
              deviceutil.restartEmulatorByName(message.emulator_name, message.port.split('-').pop(-1), message.execArgs)
            }
      })
      .handler())
