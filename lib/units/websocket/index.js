  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.DeviceScheduleUpdated, function(channel, message) {
        socket.emit('deviceSchedule.updated', {
          important: false
        , data: {
            serial: message.serial
          }
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        // device schedule
        .on('deviceSchedule.add', function(data) {
          dbapi.insertDeviceSchedule(user.email, {
            serial: data.serial
          ...
          .then(function () {
            push.send([
              wireutil.global
            , wireutil.envelope(new wire.DeviceScheduleUpdated(data.serial))
            ])
          })
        })
        .on('deviceSchedule.update', function(data) {
          dbapi.updateDeviceSchedule(user.email, {
            id: data.id
          , serial: data.serial
          , start: new Date(data.start)
          , end: new Date(data.end)
        ...
        .on('deviceSchedule.remove', function(data) {
          dbapi.deleteDeviceSchedule(data.id, user.email).then(function () {
            push.send([
              wireutil.global
            , wireutil.envelope(new wire.DeviceScheduleUpdated(data.serial))
            ])
