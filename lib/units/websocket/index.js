  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('device.wipeout', function(channel, responseChannel) {
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.WipeOutMessage()
            )
          ])
        })
        ...
        .on('admin.Add.Group', function (data) {
          dbapi.addGroup(data.serial, data.newGroup)
        })
        .on('admin.Remove.Group', function (data) {
          dbapi.removeGroup(data.serial, data.newGroup)
        })
        .on('admin.Add.User', function (data) {
          dbapi.addGroupUser(data.email, data.newGroup)
        })
        .on('admin.Remove.User', function (data) {
          dbapi.removeGroupUser(data.email, data.newGroup)
        })
        .on('get.Users', function () {
          dbapi.getUsers().then(function (data) {
        ...
        .on('load.admin.groups', function () {
          dbapi.loadGroups().then(function (data) {
            data.toArray(function (err, result) {
        ...
        .on('save.admin.group', function (data) {
          dbapi.saveGroup(data.name)
        })
        .on('delete.admin.group', function (data) {
          dbapi.deleteGroup(data.name)
        })
        .on('uploadAvatar', function (data) {
          dbapi.uploadAvatar(data.email, data.file)
        })
    })
    .finally(function() {
