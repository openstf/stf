  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('avd.restart', function(emulator_name, serial, user) {
          function getEmulatorArgsAndRestart(serial, emulator_name, output){
            dbapi.getEmulatorStartArgs(serial).then(function(args){
              var message = Object.create(null)
              message['emulator_name'] = emulator_name
              message['port'] = serial
              message['hostname'] = output.name
              message['execArgs'] = args
              push.send([ output.channel , wireutil.envelope(new wire.AVDRestart(message))])
              socket.emit('emulator.startingArgs.updated.restarted', serial, args)
              })
          }
          ...
          if (typeof(user) !== 'undefined'){
              dbapi.loadDevice(serial).then(function(device){
                  ...
                  if (proceed || (user.name === device.owner.name &&
                      user.group === device.owner.group &&
                      user.email === device.owner.email)) {
                    dbapi.getProviderInfo(device.provider.name).then(function(output){
                      joinChannel(output.channel)
                    getEmulatorArgsAndRestart(serial, emulator_name, output)
                    })
                  }else{
                    socket.emit('restart.unavailable', user, serial)
                  }
              })
          }
          else {
            dbapi.getProviderFromDevice(serial).then(function(output){
            dbapi.getProviderInfo(output.name).then(function(output){
              joinChannel(output.channel)
              getEmulatorArgsAndRestart(serial, emulator_name, output)
            })
            })
          ...
          dbapi.getDiscoveredEmulators().then(function(cursor) {
                          return Promise.promisify(cursor.toArray, cursor)()
                            .then(function(list) {
                             ...
                             socket.emit('emulator.list.received', deviceListCollectedFromDB)})
          })
        })
        .on('update.emulator.args', function(data){
          dbapi.updateEmulatorStartArgs(data.serial, data.startArgs)
          socket.emit('emulator.startingArgs.updated', data.serial, data.startArgs)
        })
    })
    .finally(function() {
