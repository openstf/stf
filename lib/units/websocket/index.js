  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        .on('screen.record', function(channel, responseChannel) {
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
              responseChannel
            , new wire.ScreenRecordMessage()
            )
          ])
        })
        .on('screen.stop.record', function(channel, responseChannel, data) {
          joinChannel(responseChannel)
          push.send([
            channel
          , wireutil.transaction(
              responseChannel
            , new wire.ScreenStopRecordMessage(data)
            )
          ])
        })
