...
// TODO refactor this into a proper unit without the dependency to this websocket unit
var test = require('../testexecutor/index.js')
  ...
  io.on('connection', function(socket) {
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('testing.tool.remove', function(tool) {
          return dbapi.removeTestingTool(user.email, tool)
            .then(function() {
              socket.emit('testing.tool.removed', tool)
            })
        })
        // TODO refactor this into the testexecutor unit
        .on('command.testing.tools.execute', function(tests, storageId) {
          test.executeTests(socket, user.email, tests, storageId, options)
        })
