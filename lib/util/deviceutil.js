...
deviceutil.restartEmulatorByName = function (emulator_name, port, execArgs) {
  ...
  if (devutil.ANDROIDSDKFileExists(process.env.ANDROID_HOME + '/tools/' + binaryOSName) === true) {
    ...
    function startEmulator(sdk_path, binaryOSName, name, port) {
        var cmd = sdk_path + binaryOSName + ' -avd ' + name + ' -port ' + port + ' ' + execArgs
        cmdExec.get(
            cmd,
            function(err, data, stderr) {
