...
devutil.ANDROIDSDKFileExists =  function(path) {
    try {
      return fs.statSync(path).isFile();
    } catch (err) {
      if (err.code === 'ENOENT') {
        log.info('Wrong path to ANDROID_SDK. Does not contain android, emulator binaries. Emulator functionality might to be disabled on this machine. Please check your configuration')
        return false
      }
      log.info('Exception fs.statSync (' + path + '): ' + e);
      return false
    }
  }
