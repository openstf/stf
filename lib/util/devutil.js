...
devutil.RestartEmulatorByName = function (emulator_name, serial) {
  ...
  if (pathExists(process.env.ANDROID_HOME + '/tools/' + emulatorOSName) === true) {
    ...
    function startEmulator(sdk_path, emulatorOSName, name, serial) {
        var cmd = sdk_path + emulatorOSName + ' -avd ' + name + ' -port ' +  serial
        cmdExec.get(
            cmd,
    ...
    startEmulator(androidSdkPath, emulatorOSName, emulator_name, serial)
  }
}
