...
devutil.getAVDName = function (cmd, serial) {
...
devutil.RestartEmulatorByName = function (emulator_name) {
  ...
  var emulatorOSName = 'emulator';
  if ( os.type() === 'Windows_NT' ) {
      emulatorOSName = 'emulator.exe'
  }

  if (pathExists(process.env.ANDROID_HOME + '/tools/' + emulatorOSName) === true) {
    var androidSdkPath = process.env.ANDROID_HOME + '/tools/'
    var cmdExec=require('node-cmd');
    function startEmulator(sdk_path, emulatorOSName, name) {
        var cmd = sdk_path + emulatorOSName + ' -avd ' + name
        cmdExec.get(
            cmd,
    ...
    startEmulator(androidSdkPath, emulatorOSName, emulator_name)
  }
}
