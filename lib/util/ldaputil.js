...
// Export
module.exports.checkUserMembership = function (ldapAllowedGroups, currentUserGroups) {
  var isMemberAllowed = true;
  if (ldapAllowedGroups) {
    var ldapAllowedGroupsList = ldapAllowedGroups.split(', ');
    var currentUserGroupsList = typeof currentUserGroups === 'string' ? currentUserGroups.split(',') :
      currentUserGroups.join(',').split(',');
    isMemberAllowed = ldapAllowedGroupsList.some(function (group) {
      return currentUserGroupsList.indexOf(group) >= 0
    });
  }
